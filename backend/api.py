...
from utils.rate_limiter import get_rate_limiter, check_rate_limit, start_request_tracking
...
class RateLimitExceededError(APIError):
    ...
    def __init__(self, message: str = "Rate limit exceeded. Please try again later."):
        super().__init__(message, error_type="rate_limit_exceeded", status_code=429)
...
async def get_company_intelligence(name: str):
    ...
    if not check_rate_limit():
        logger.warning(f"Rate limit exceeded for request: {name}")
        raise RateLimitExceededError()
    
    # Reset API call counter for this new request
    ...
    # Process request
    try:
        result = graph.invoke({"company": validated_name})
