...
class TestResearchAgent:
    ...
    @patch('agents.research_agent.requests.get')
    def test_scrape_wikipedia_success(self, mock_get):
        """Test Wikipedia scraping"""
        from agents.research_agent import scrape_wikipedia
        
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"extract": "Test company description"}
        mock_get.return_value = mock_response
        
        result = scrape_wikipedia("TestCompany")
        
        assert "Test company description" in result

    @patch('agents.research_agent.requests.get')
    def test_scrape_wikipedia_not_found(self, mock_get):
        ...
        result = scrape_wikipedia("NonexistentCompany")
